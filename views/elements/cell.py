...
class Cell:
    def __init__(self,
        ...
        self.normal_image = pygame.transform.scale(
                            pygame.image.load(GAME_SPRITES['cell_surf']), (bounds.w, bounds.h)
                            )
        self.hover_image = pygame.transform.scale(
                            pygame.image.load(GAME_SPRITES['cell_surf_select']), (bounds.w, bounds.h)
                            )

    @property
    def back_image(self):
        return dict(normal=self.normal_image,
                    hover=self.hover_image,
                    pressed=self.image)[self.state]
